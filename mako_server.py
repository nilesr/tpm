...
def serve(environ, start_response, **kwargs):
	new_environ = environ
	for module in moduleObjects:
		try:
			returnvalue, new_environ = module.onRequest(start_response=start_response,environ=new_environ,log=log,logfile=logfile,root=root,serverError=serverError,config=config,file=__file__,getfield=getfield, extras=kwargs)
			if returnvalue:
				return returnvalue
		except:
...
#**
#* Loads modules from a directory into a list
#* @author			Niles Rogoff <nilesrogoff@gmail.com>
#* @version                     092
#* @since                       2014-11-17
#* @params                      string directory, iterable list of strings of modules to load
#* @returns                     list of modules

def load_modules(directory, modlist):
	sys.path.append(directory)
	try:
		moduleObjects = list(map(__import__, modlist))
	except:
		log("Fatal error: One or more modules failed to import. Please check your config file, and each module file")
		sys.exit(1)
	for module in moduleObjects:
		module.onLoad(log=log,logfile=logfile,serverError=serverError,config=config,file=__file__,getfield=getfield)
	return moduleObjects

logfile = config = root = port = ssl = ssl_port = bind_host = ssl_pem_file = server_version_string = render_timeout = threads = request_queue_size = False
...
#	configfile = os.path.dirname(os.path.realpath(__file__)) + "/config.conf"
	...
	moduleObjects = load_modules(os.path.dirname(os.path.realpath(__file__))+'/modules', modules)
